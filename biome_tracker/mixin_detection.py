...
class DetectionMixin:
    ...
    def get_total_session_time(self):
        try:
            ...
            if self.start_time:
                ...
                # current_session is elapsed seconds for this run. It used to
                # increment once per UI refresh, which made a 5-hour run appear
                # as roughly 48 hours when refreshes happened many times/second.
                self.current_session = max(0, elapsed_time)
            else:
    ...
    def stop_detection(self):
        ...
        session_seconds = max(0, elapsed_time)
        # Freeze the exact duration of this run before the summary is created.
        self.current_session = session_seconds
                    ...
                    if match:
                        ...
                        if real_aura_key:
                            ...
                            if parsed_aura_name != self.last_aura_found:
                                ...
                                try:
                                    overlay_minimum = max(1, int(self.config.get("overlay_minimum_rarity", 1) or 1))
                                    if int(rarity) >= overlay_minimum:
                                        self.overlay_last_aura_found = parsed_aura_name
                                        self.overlay_last_aura_rarity = f"1 in {formatted_rarity}"
                                except Exception:
                                    pass
                            ...
                            if aura != self.last_aura_found:
                                ...
                                try:
                                    overlay_minimum = max(1, int(self.config.get("overlay_minimum_rarity", 1) or 1))
                                    if overlay_minimum <= 1:
                                        self.overlay_last_aura_found = aura
                                        self.overlay_last_aura_rarity = "Unknown"
                                except Exception:
                                    pass
