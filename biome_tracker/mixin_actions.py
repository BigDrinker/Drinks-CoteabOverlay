...
class ActionsMixin:
    ...
    def _scheduled_merchant_ocr_check(self):
        try:
            ...
            detected_merchant = found_merchant.title()
            self.overlay_last_merchant_found = detected_merchant
            self.overlay_last_merchant_detected_at = time.time()
            print(f"[Merchant OCR] {detected_merchant} spawn detected!")
            self.append_log(f"[Merchant OCR] {detected_merchant} spawn detected!")
            screenshot_path = None
            try:
                if self.is_roblox_focused():
                    x, y, w, h = int(chat_box_region[0]), int(chat_box_region[1]), int(chat_box_region[2]), int(chat_box_region[3])
    ...
    def Merchant_Handler(self):
        try:
            ...
            if merchant_name:
                # This is the merchant accepted by the Merchant tab's own OCR
                # flow, so publish that exact result to the overlay.
                self.overlay_last_merchant_found = merchant_name
                self.overlay_last_merchant_detected_at = time.time()
