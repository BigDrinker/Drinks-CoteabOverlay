...
class WebhookMixin:
    ...
    def get_webhook_list(self):
        """Return normalized, unique webhook URLs while preserving order."""
        try:
            raw = self.webhook_urls if (hasattr(self, "webhook_urls") and isinstance(self.webhook_urls, list)) else self.config.get("webhook_url", "")
            if isinstance(raw, str):
                ...
                if not value:
                    ...
                    try:
                        ...
                        candidates = parsed if isinstance(parsed, list) else [value]
                    except Exception:
                        candidates = [value]
    ...
    def send_webhook(self, biome, message_type, event_type, screenshot_path=None):
        # Multiple detector paths can report the same transition almost
        # simultaneously. Suppress only an identical biome/event duplicate
        # within a short window; legitimate later starts/ends still send.
        now_mono = time.monotonic()
        event_key = (str(biome).upper().strip(), str(event_type).lower().strip())
        lock = getattr(self, "_biome_webhook_dedupe_lock", None)
        if lock is None:
            lock = threading.Lock()
            self._biome_webhook_dedupe_lock = lock
        with lock:
            recent = getattr(self, "_biome_webhook_recent", {})
            previous = float(recent.get(event_key, 0.0) or 0.0)
            if now_mono - previous < 8.0:
                try:
                    self.append_log(f"Skipped duplicate webhook for {event_key[0]} - {event_key[1]}")
                except Exception:
                    pass
                return
            recent[event_key] = now_mono
            self._biome_webhook_recent = {k: v for k, v in recent.items() if now_mono - v < 120.0}
