...
class LifecycleMixin:
    def __init__(self):
        ...
        # Overlay values are updated by the same detector paths used by the
        # Biomes, Auras, and Merchant tabs. Keeping them on the tracker
        # prevents the overlay from running a separate, conflicting detector.
        self.overlay_last_aura_found = None
        self.overlay_last_aura_rarity = None
        self.overlay_last_merchant_found = None
        self.overlay_last_merchant_detected_at = None
        ...
        # A session belongs only to the current application run. Do not restore
        # the previous run's elapsed time from config when the macro reopens.
        self.saved_session = 0
        self._session_window_reset_performed = False
        self.current_session = 0
        self.session_window_start = datetime.now()
        self.config["session_time"] = "00:00:00"
        self.config["session_window_start"] = self.session_window_start.isoformat()
        self.config["macro_last_start"] = ""
